...
| **Claude driver + ephemeral codex workers** (Round 15p) | `agents.<topology>.yaml` carries a `runtimes:` section declaring per-agent runtime (`runtime: claude` / `runtime: codex`). cmdRun's `<dispatch peer="X">` directive spawns a fresh `codex exec` (or `claude -p`) child as agent X — no long-running interactive lumeyon terminal needed. **Phase A empirical proof: 16/16 PASS** on real cross-runtime turn round-trip. Optional headless mode via `agent-chat autowatch` + systemd installer; the long-running Codex app-server proposal was empirically derisked but ultimately rejected (`docs/codex-live-app-server-proposal.md`) because ephemeral-spawn covers the use case with strictly simpler architecture. |
