...
| `scripts/lattice/sqlite-store.ts` | keystone (codex) | 2026-05-07 | reviewed | 3 raised: K1 best_answer_id not FK-validated and not checked for existence/matching/accepted (queued iter 7, runtime-guard part — schema FK migration pending boss approval); K2 CHECK quality_tier BETWEEN 1 AND 5 allows fractional (e.g., 2.5) — schema migration pending boss approval; K3 DAG cycle check + insert not atomic (race condition — queued iter 8, atomic via BEGIN IMMEDIATE) | none — keystone returned all 3 as REAL findings, no nitpicks | iter-6 |
